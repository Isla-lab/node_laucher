...
This repo contains the instructions and code to access and run your processes on the ISLa mini-cluster. Please read the instructions below carefully before proceeding. If you have any questions or errors, contact alessandro.farinelli@univr.it.
...
Contact alberto.castellini@univr.it to obtain credentials for one of the cluster nodes. In the email, specify whether you intend to use GPU or CPU only. The administrator, given the availability of nodes, will provide you with a username, password, and the IP address of the node assigned. 
...
> 
> If you need to run any particular script other than Python, send an email to alberto.castellini@univr.it.
...
#### Create a Python virtual environment and install dependencies
 ...
 You can create various Python virtual environments to have 100% compatibility with your personal computer. These environments work similarly to *Conda*. 
...
In order to use the mini-cluster, once you enter in ssh in the node, 
...
Screen or GNU Screen is a terminal multiplexer. In other words, it means that you can start a screen session and then open any number of windows (virtual terminals) inside that session. Processes running on Screen will continue to run when their window is not visible, even if you get disconnected.
**DO NOT use ctrl+C to stop your script, otherwise you will block the priority queue!**
...
This script creates a log file where any errors will be saved. It then prints on the screen the parameters that are passed to the script. Please notice that it is good practice to put your code enclosed in *try-except* commands in order to figure out why your script possibly does not work.
We then create the *.txt* configuration file to run our script on the assigned node. Specifically, suppose we want to use only the CPU and send 2 executions in parallel at a time. So we write the file as:
...
This command will run two rows of your config.txt in parallel using the CPU until the end of the file.
...
To detach from the screen, use ctrl+a and ctrl+d. 
